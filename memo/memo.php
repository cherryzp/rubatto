		...
		<div id="content">
			...
			<section id="col2">
				...
				<?php 
					...
					// 전체 저장 데이터들(레코드row들) 읽어오기 가장 최근글이 먼저 나오도록 정렬(num 내림차순)..
					...
					// 총 페이지 수(한 페이지에 5($scale)개 메모글)
					// (1~5: 1page, 6~10: 2page, 11~15: 3page...)
					$pageNum= ceil($rowNum/$scale);
					if($pageNum==0) $pageNum=1;	// 게시글 0이면 1page

					// 페이지 수 많기에 현재 페이지가 몇 페이지인지 알 필요가 있음.
					// 이 html의 마지막에 페이지 번호가 있고 이를 선택했을때 page값을 GET 방식으로 memo.php를 다시 로드.
					$page= $_GET['page'];
					if(!$page) $page= 1;

					// 현재 페이지($page)에서 보여줄 5($scale)의 메모글 출력하기.
					...
					for($i= $start; $i<$start+$scale && $i<$rowNum; $i++){
						...
						// 읽어온 값들을 출력!!
						// 모양설계를 위해 html문법 사용하기위해 php문법 분리
				?>
